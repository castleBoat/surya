...
from surya.translate import draw_orc_on_images
import torch
import os
...
def main():
    ...
    parser.add_argument(
        "--langs",
        type=str,
        help="Language(s) to use for OCR. Comma separate for multiple. Can be a capitalized language name, or a 2-letter ISO 639 code.",
        default=None,
    )
    parser.add_argument(
        "--lang_file",
        type=str,
        help="Path to file with languages to use for OCR. Should be a JSON dict with file names as keys, and the value being a list of language codes/names.",
        default=None,
    )
    parser.add_argument(
    ...
    predictions_by_image = run_ocr(
        images, image_langs, line_predictions, rec_model, rec_processor
    )

    layout_model = load_detection_model(checkpoint=settings.LAYOUT_MODEL_CHECKPOINT)
    layout_processor = load_detection_processor(
    ...
    if args.images:
        for idx, (name, image, pred, langs) in enumerate(
            ...
            page_image = draw_text_on_image(
                bboxes, pred_text, image.size, langs, has_math="_math" in langs
            )
    ...
    with open(
        os.path.join(result_path, "ocr_results.json"), "w+", encoding="utf-8"
    ) as f:
    ...
    with open(
        os.path.join(result_path, "layout_results.json"), "w+", encoding="utf-8"
    ) as f:
